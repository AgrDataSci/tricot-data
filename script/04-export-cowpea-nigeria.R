...
# funders 
funders = read_excel('metadata/trials-metadata/cowpea-iita/funder-metadata.xlsx')
...
trial_description = paste("On-farm trials implemented under the scope of", 
                          "AVISA and 1000FARMS projects to test new", 
                          "cowpea varieties on-farm.")
...
crop_taxon = "Vigna unguiculata"

# filter the worlveg projects
keep = grep("Ousmane Boukar|Patrick Ongom", projects$coordinator)
...
for(k in seq_along(cmdata)) {
  ...
  block = exportBlockData(x, seed = 1557)
